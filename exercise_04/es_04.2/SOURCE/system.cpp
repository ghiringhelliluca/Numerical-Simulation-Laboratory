...
// debug function to print the particle positions
void System :: debug(){
  ...
  for (int i = 0; i < _npart; i++) {
      double x = _particle(i).getposition(0, true);
      double y = _particle(i).getposition(1, true);
      double z = _particle(i).getposition(2, true);
  ...
  std::cout << "Particle Position Range: \n";
...
void System :: step(){ // Perform a simulation step
  ...
  _nattempts += _npart; //update number of attempts performed on the system, denom in calcolo accettazione
  return;
}

void System :: Verlet(){
  double xnew, ynew, znew;
  for(int i=0; i<_npart; i++){ //Force acting on particle i, force = a because mass is 1
...
double System :: Force(int i, int dim){
  ...
  for (int j=0; j<_npart; j++){
    if(i != j){
      ...
      // check cutoff radius for LJ potential
      if(dr < _r_cut){
...
void System :: move(int i){ // Propose a MC move for particle i
  if(_sim_type == 3){ //Gibbs sampler for Ising, metto metodo di GIBBS, valore di spin che gibbs vuole dare, con accettazione sempre 1
...
bool System :: metro(int i){ // Metropolis algorithm
  ...
  if(_sim_type == 1) delta_E = this->Boltzmann(i,true) - this->Boltzmann(i,false);
                 ...
                 ( _J * (_particle(this->pbc(i-1)).getspin() + _particle(this->pbc(i+1)).getspin() ) + _H );  // pbc sono quelli di una catena N+1 = 1
...
void System :: initialize_velocities(){
  ...
  if(_restart){ // if restart simulation from a previous run, read coord from conf-1.xyz
    ifstream cinf;
    ...
    vec sumv(_ndim);
    sumv.zeros();
    double vt = sqrt(3 * _temp); // average velocity for a given temperature

    for (int i = 0; i < _npart; i++) {

      int axis = i % 3; // 0 -> x, 1 -> y, 2 -> z
      int sign = (i % 2 == 0) ? 1 : -1; // alternating sign for each axis
...
void System :: write_configuration(){
  ...
  if(_sim_type < 2){
    ...
    }  else cerr << "PROBLEM: Unable to open config.vel" << endl; 
    coutf.close();

    // save coordinate and velocity for 4.3 exercise
    coutf.open("../../es_04.3/INPUT/CONFIG/config.fcc");
    if(coutf.is_open()){
      coutf << _npart << endl;
      coutf << "#Comment!" << endl;
      for(int i=0; i<_npart; i++){
    ...
    coutf.close();
    coutf.open("../../es_04.3/INPUT/CONFIG/conf-1.fcc");
    if(coutf.is_open()){
      coutf << _npart << endl;
      coutf << "#Comment!" << endl;
      for(int i=0; i<_npart; i++){
    ...
    coutf.close();
    coutf.open("../../es_04.3/INPUT/CONFIG/config.vel");
    if(coutf.is_open()){
      coutf << _npart << endl;
      coutf << "#Comment!" << endl;
      for(int i =0; i < _npart; i++){
...
void System :: read_configuration(){
  ...
  if(cinf.is_open()){
    ...
    for(int i=0; i<_npart; i++){
      ...
      // particles occupy half of the box, multiply by _halfside

      //  from [-0.5, 0.5] to [0,1]
      //  from  [0,1] to [0, _side/2]
      double new_x = (_side(0) / 2.0) * (x + 0.5);
      double new_y = (_side(1) / 2.0) * (y + 0.5);
      double new_z = (_side(2) / 2.0) * (z + 0.5);
      
      // new_x, new_y, new_z are in [0, _side/2]
      _particle(i).setposition(0, new_x - _halfside(0));
      _particle(i).setposition(1, new_y - _halfside(1));
      _particle(i).setposition(2, new_z - _halfside(2));
...
void System :: measure(){ // Measure properties
  ...
  if(_measure_pofv){
    ...
    double vmod;
    int bin;
    // accumulate bins count
    for (int i = 0; i < _npart; i++){
      vmod = sqrt( dot( _particle(i).getvelocity(), _particle(i).getvelocity() ) );
      // bin for velocity vmod
      bin = int(vmod / _bin_size_v);
      // verify if bin is in the range of the bins
      if(bin >= 0 && bin < _n_bins_v)
        _measurement(_index_pofv + bin) += 1.0;
    }
    
    // normalization of the bins by the number of particles and the bin size
