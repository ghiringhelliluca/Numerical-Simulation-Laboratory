...
void System :: initialize_velocities(){
  ...
  if(_restart){ // load configuration of coord-1 and velocities from previous simulation
    ...
    if(cinf.is_open()){
      ...
      for(int i=0; i<_npart; i++){
        ...
        _particle(i).setposition(0, this->pbc(_side(0)*xold, 0)); 
        _particle(i).setposition(1, this->pbc(_side(1)*yold, 1));
        _particle(i).setposition(2, this->pbc(_side(2)*zold, 2));
      }
    } else cerr << "PROBLEM: Unable to open INPUT file conf-1.fcc"<< endl;
    cinf.close();

    cinf.open("../INPUT/CONFIG/config.vel");
    if(cinf.is_open()){
      string comment;
      string particle;
      int ncoord;
      ...
      double vx, vy, vz;
      for(int i=0; i<_npart; i++){
        cinf >> particle >> vx >> vy >> vz; // take last velocities and change velocities sign
        _particle(i).setvelocity(0, - vx); 
        _particle(i).setvelocity(1, - vy);
        _particle(i).setvelocity(2, - vz);
      }
    } else cerr << "PROBLEM: Unable to open INPUT file config.vel"<< endl;
    cinf.close();
  } else { // start from scratch
    vec vx(_npart), vy(_npart), vz(_npart);
    vec sumv(_ndim);
    sumv.zeros();
    double vt = sqrt(3 * _temp); 
...
void System :: read_configuration(){
  ifstream cinf;
  cinf.open("../INPUT/CONFIG/config.fcc"); // Read configuration from file
  // file contains the last coordinates of the particles of 04.2 exercise simulation
  if(cinf.is_open()){
    string comment;
    string particle;
    double x, y, z;
...
// metto valori istantanei in _measurment che servono poi per blocking
