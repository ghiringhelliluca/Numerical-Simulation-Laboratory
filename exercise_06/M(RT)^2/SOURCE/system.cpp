...
void System :: Verlet(){
  ...
  for(int i=0; i<_npart; i++){ //Force acting on particle i, forza = a perchè massa è 1
    _fx(i) = this->Force(i,0);
    _fy(i) = this->Force(i,1);
    _fz(i) = this->Force(i,2);
  }
  for(int i=0; i<_npart; i++){ //Verlet integration scheme
    // true = x(t), false = x(t - delta t), xnew = x(t + delta t)
...
double System :: Force(int i, int dim){
  ...
  for (int j=0; j<_npart; j++){
    if(i != j){
      ...
      // controllo per cut off potenziale
      if(dr < _r_cut){
...
void System :: move(int i){ // Propose a MC move for particle i
  if(_sim_type == 3){ //Gibbs sampler for Ising 1D
    // dE = 2 * (J * s(i) * (s(i-1) + s(i+1)) + H * s(i))
		double delta_E =  2. * (_J * (_particle(this->pbc(i-1)).getspin() + _particle(this->pbc(i+1)).getspin()) + _H);  // s(i) = +1
		double prob_i_up = 1. / (1. + exp (-1.*_beta * delta_E)); // conditional distribution spin up (s(i) = +1))
...
bool System :: metro(int i){ // Metropolis algorithm
  ...
  if(_sim_type == 1) delta_E = this->Boltzmann(i,true) - this->Boltzmann(i,false);
                 ...
                 ( _J * (_particle(this->pbc(i-1)).getspin() + _particle(this->pbc(i+1)).getspin() ) + _H );  // pbc of a 1D ising model
...
void System :: initialize(){ // Initialize the System object according to the content of the input files in the ../INPUT/ director
  ...
  while ( !input.eof() ){
    ...
    if( property == "SIMULATION_TYPE" ){
      ...
      input >> _temp;
      _beta = 1.0/_temp;
      coutf << "TEMPERATURE START= " << _temp << endl;
...
// initialize the system to compute properties for studying equilibration
void System :: initialize_equilibration(){ // Initialize the System object according to the content of the input files in the ../INPUT/ director
  int p1, p2; // Read from ../INPUT/Primes a pair of numbers to be used to initialize the RNG
  ifstream Primes("../INPUT/Primes");
  Primes >> p1 >> p2 ;
  Primes.close();
  ...
  ofstream couta("../OUTPUT/EQUILIBRATION/acceptance.dat"); // Set the heading line in file ../OUTPUT/EQUILIBRATION/acceptance.dat
  couta << "#   STEP:  ACCEPTANCE:" << endl;
  couta.close();

  ifstream input("../INPUT/input.dat"); // Start reading ../INPUT/input.dat
  ofstream coutf;
  coutf.open("../OUTPUT/EQUILIBRATION/output.dat");
  string property;
  double delta;
  while ( !input.eof() ){
    input >> property;
    if( property == "SIMULATION_TYPE" ){
      ...
      if(_sim_type == 0)      coutf << "LJ MOLECULAR DYNAMICS (NVE) EQUILIBRATION"  << endl;
      else if(_sim_type == 1) coutf << "LJ MONTE CARLO (NVT) EQUILIBRATION"         << endl;
      else if(_sim_type == 2) coutf << "ISING 1D MONTE CARLO (MRT^2) EQUILIBRATION" << endl;
      else if(_sim_type == 3) coutf << "ISING 1D MONTE CARLO (GIBBS) EQUILIBRATION" << endl;
    } else if( property == "RESTART" ){
      input >> _restart;
    } else if( property == "TEMP" ){
      input >> _temp;
      ...
      input >> _nblocks;
      coutf << "NBLOCKS not used in EQUILIBRATION" << endl;
    } else if( property == "NSTEPS" ){
      input >> _nsteps;
      coutf << "NSTEPS not used in EQUILIBRATION" << endl;
    } else if( property == "ENDINPUT" ){
      coutf << "Reading input completed!" << endl;
      break;
    } else cerr << "PROBLEM: unknown input" << endl;
...
void System :: initialize_velocities(){
  ...
  if(_restart){ // if restart from previous simulation, take previous positions
    ifstream cinf;
    cinf.open("../INPUT/CONFIG/conf-1.fcc");
    if(cinf.is_open()){
      string comment;
      string particle;
      int ncoord;
      ...
      cinf >> comment;
      for(int i=0; i<_npart; i++){
        cinf >> particle >> xold >> yold >> zold; // units of coordinates in conf-1.fcc is _side
        // old coordinates sono espresse in funzione di side xold = cord(x)/_side(x)
    ...
    } else cerr << "PROBLEM: Unable to open INPUT file conf-1.fcc"<< endl;
    cinf.close();
  } else { // restart simulation from scratch
    vec vx(_npart), vy(_npart), vz(_npart);
    vec sumv(_ndim);
    sumv.zeros();
    double vt = sqrt(3 * _temp); // Velocità quadratica media

    for (int i = 0; i < _npart; i++) {

      int axis = i % 3; // 0 -> x, 1 -> y, 2 -> z
      int sign = (i % 2 == 0) ? 1 : -1; // Alterna segno
...
void System :: initialize_properties(){ // Initialize data members used for measurement of properties
  ...
  if (_H == 0.0) {
    properties_file = "../INPUT/properties_H=0.dat";
  } else {
    properties_file = "../INPUT/properties_H!=0.dat";
  }
  ifstream input(properties_file);
  if (input.is_open()){
    while ( !input.eof() ){
      input >> property;
      if( property == "POTENTIAL_ENERGY" ){
        ...
        coutpv << "# BLOCK     VELOCITY     AVE_POFV     CUM_AVG_POFV     ERROR" << endl;
        coutpv.close();
        input>>_n_bins_v;
        _nprop += _n_bins_v;
...
// initialize properties to compute during equilibration studying process
void System :: initialize_properties_equilibration(){ // Initialize data members used for measurement of properties
  ...
  if (input.is_open()){
    while ( !input.eof() ){
      input >> property;
      if( property == "POTENTIAL_ENERGY" ){
        ofstream coutp("../OUTPUT/EQUILIBRATION/potential_energy.dat");
        coutp << "#     STEP:  MEASURE:" << endl;
        coutp.close();
        _nprop++;
        _index_penergy = index_property;
        _measure_penergy = true;
        ...
        ofstream coutk("../OUTPUT/EQUILIBRATION/kinetic_energy.dat");
        coutk << "#     STEP:   MEASURE:" << endl;
        coutk.close();
        _nprop++;
        _measure_kenergy = true;
        _index_kenergy = index_property;
        ...
        ofstream coutt("../OUTPUT/EQUILIBRATION/total_energy.dat");
        coutt << "#     STEP:   MEASURE:" << endl;
        coutt.close();
        _nprop++;
        _measure_tenergy = true;
        _index_tenergy = index_property;
        ...
        ofstream coutte("../OUTPUT/EQUILIBRATION/temperature.dat");
        coutte << "#     STEP:   MEASURE:" << endl;
        coutte.close();
        _nprop++;
        _measure_temp = true;
        _index_temp = index_property;
        ...
        ofstream coutpr("../OUTPUT/EQUILIBRATION/pressure.dat");
        coutpr << "#     STEP:   MEASURE:" << endl;
        coutpr.close();
        _nprop++;
        _measure_pressure = true;
        _index_pressure = index_property;
        ...
        ofstream coutgr("../OUTPUT/EQUILIBRATION/gofr.dat");
        coutgr << "# DISTANCE:     MEASURE:" << endl;
        coutgr.close();
        input>>_n_bins;
        _nprop+=_n_bins;
        _bin_size = (_halfside.min() )/(double)_n_bins;
        ...
        ofstream coutpv("../OUTPUT/EQUILIBRATION/pofv.dat");
        coutpv << "# BLOCK     VELOCITY     MEASURE" << endl;
        coutpv.close();
        input>>_n_bins_v;
        _nprop += _n_bins_v;
        _bin_size_v = 4.0* sqrt(_temp)/(double)_n_bins_v; 
        ...
        ofstream coutpr("../OUTPUT/EQUILIBRATION/magnetization.dat");
        coutpr << "#     STEP:   MEASURE:" << endl;
        coutpr.close();
        _nprop++;
        _measure_magnet = true;
        ...
        ofstream coutpr("../OUTPUT/EQUILIBRATION/specific_heat.dat");
        coutpr << "#     STEP:   MEASURE:" << endl;
        coutpr.close();
        _nprop++;
        _measure_cv = true;
        ...
        ofstream coutpr("../OUTPUT/EQUILIBRATION/susceptibility.dat");
        coutpr << "#     STEP:   MEASURE:" << endl;
        coutpr.close();
        _nprop++;
        _measure_chi = true;
        ...
        ofstream coutf;
        coutf.open("../OUTPUT/EQUILIBRATION/output.dat",ios::app);
        coutf << "Reading properties completed!" << endl;
        coutf.close();
        break;
      } else cerr << "PROBLEM: unknown property" << endl;
...
void System :: initialize_properties_output(){ // Initialize data members used for measurement of properties and output file for final mean blocking values
...
// finalize the simulation for each T
void System :: finalize(double T){
  this->write_configuration();
  _rnd.SaveSeed();
  ofstream coutf;
  coutf.open("../OUTPUT/output.dat",ios::app);
  coutf << "Simulation T = " << T <<" completed!" << endl;
  coutf.close();
  _average.zeros();
  _global_av.zeros();
  _global_av2.zeros();
...
void System :: finalize_equilibration(){
  this->write_configuration();
  _rnd.SaveSeed();
  ofstream coutf;
  coutf.open("../OUTPUT/EQUILIBRATION/output.dat",ios::app);
  coutf << "Equilibration completed!" << endl;
  coutf.close();
  return;
}
...
void System :: write_configuration(){
  ofstream coutf;
  if(_sim_type < 2){
    coutf.open("../OUTPUT/CONFIG/config.fcc");
    if(coutf.is_open()){
      coutf << _npart << endl;
      coutf << "#Comment!" << endl;
      for(int i=0; i<_npart; i++){
    ...
    } else cerr << "PROBLEM: Unable to open config.fcc" << endl;
    coutf.close();
    coutf.open("../OUTPUT/CONFIG/conf-1.fcc");
    if(coutf.is_open()){
      coutf << _npart << endl;
      coutf << "#Comment!" << endl;
      for(int i=0; i<_npart; i++){
    ...
    } else cerr << "PROBLEM: Unable to open conf-1.fcc" << endl;
    coutf.close();

    // save velocity for 4.3 exercise
    coutf.open("../OUTPUT/CONFIG/config.vel");
    if(coutf.is_open()){
      coutf << _npart << endl;
      coutf << "#Comment!" << endl;
      for(int i =0; i < _npart; i++){
        coutf << "LJ" << "  "
              << setprecision(17) << _particle(i).getvelocity(0) << "   " // vx
              << setprecision(17) << _particle(i).getvelocity(1) << "   " // vy
              << setprecision(17) << _particle(i).getvelocity(2) << endl; // vz      
      }
    }
  } else {
    coutf.open("../OUTPUT/CONFIG/config.spin"); // poi lo metto anche in /INPUT/CONFIG così da non dover ri iniaziare la simulazione da capo
...
void System :: write_XYZ(int nconf){
  ofstream coutf;
  coutf.open("../OUTPUT/CONFIG/config_" + to_string(nconf) + ".fcc");
  if(coutf.is_open()){
    coutf << _npart << endl;
    coutf << "#Comment!" << endl;
    for(int i=0; i<_npart; i++){
...
void System :: read_configuration(){
  ...
  if(cinf.is_open()){
    ...
    cinf >> comment;
    for(int i=0; i<_npart; i++){
      cinf >> particle >> x >> y >> z; // units of coordinates in conf.ising is _side
      
      _particle(i).setposition(0, x * _side(0));
      _particle(i).setposition(1, y * _side(1));
      _particle(i).setposition(2, z * _side(2));
  ...
  if(!_restart and _sim_type > 1){
		for(int i=0; i<_npart; i++){
			if (_rnd.Rannyu() < 0.5) _particle(i).setspin(-1); // random spin configuration, T = inf
			else _particle(i).setspin(1);
		}
	}
...
// _measurement is a vector of size _nprop, where each index represents a physical quantity to be measured
// at the end I sum them in _block_av which keeps the block averages
...
void System :: measure(){ // Measure properties
  ...
  double tenergy_temp=0.0;  // total energy 
  ...
  // POFV //////////////////////////////////////////////////////////////////////////
  if(_measure_pofv){
    for (int b = 0; b < _n_bins_v; ++b) {
      _measurement(_index_pofv + b) = 0.0;
    }

    double vmod;
    int bin;
    for (int i = 0; i < _npart; i++){
      vmod = sqrt( dot( _particle(i).getvelocity(), _particle(i).getvelocity() ) );
      bin = int(vmod / _bin_size_v);
      if(bin >= 0 && bin < _n_bins_v)
        _measurement(_index_pofv + bin) += 1.0;
    }
    
    for (int b = 0; b < _n_bins_v; ++b) {
      _measurement(_index_pofv + b) /= ( _npart * _bin_size_v );
    }
  }
  ...
  // MAGNETIZATION /////////////////////////////////////////////////////////////
  if (_measure_magnet)  _measurement(_index_magnet) = spin_sum / double(_npart);
  // SPECIFIC HEAT /////////////////////////////////////////////////////////////
  if(_measure_cv and _measure_tenergy)  _measurement(_index_cv) = pow(_measurement(_index_tenergy) * _npart,2)  ; // total energy squared
  // SUSCEPTIBILITY //////////////////////////////////////////////////////////// 
  if(_measure_chi)  _measurement(_index_chi) = _beta * pow(spin_sum ,2) / _npart;

  _block_av += _measurement; //Update block accumulators with the new measurements

  return;
}


// measure equilibration properties and save for each step in /OUTPUT/EQUILIBRATION/{prop}.dat 
void System :: measure_equilibration(int step){ // Measure properties
  ...
  if (_measure_penergy){
    penergy_temp = _vtail + 4.0 * penergy_temp / double(_npart);
    ofstream coutp("../OUTPUT/EQUILIBRATION/potential_energy.dat",ios::app);
    coutp << setw(12) << step
          << setw(12) << penergy_temp << endl;
  }
  // KINETIC ENERGY ////////////////////////////////////////////////////////////
  if (_measure_kenergy){
    for (int i=0; i<_npart; i++) kenergy_temp += 0.5 * dot( _particle(i).getvelocity() , _particle(i).getvelocity() ); 
    kenergy_temp /= double(_npart);
    ofstream coutk("../OUTPUT/EQUILIBRATION/kinetic_energy.dat",ios::app);
    coutk << setw(12) << step
          << setw(12) << kenergy_temp << endl;
  }
  // TOTAL ENERGY (kinetic+potential) //////////////////////////////////////////
  if (_measure_tenergy){
    ...
    _accu_tenergy += tenergy_temp;
    ofstream coutt("../OUTPUT/EQUILIBRATION/total_energy.dat",ios::app);
    coutt << setw(12) << step
          << setw(12) << tenergy_temp << endl;
  }
  // TEMPERATURE ///////////////////////////////////////////////////////////////
  if (_measure_temp and _measure_kenergy){
    ofstream coutte("../OUTPUT/EQUILIBRATION/temperature.dat",ios::app);
    coutte << setw(12) << step
          << setw(12) << (2.0/3.0) * kenergy_temp << endl;
  }
  // PRESSURE //////////////////////////////////////////////////////////////////
  if (_measure_pressure){
    ofstream coutpr("../OUTPUT/EQUILIBRATION/pressure.dat",ios::app);
    coutpr << setw(12) << step
          << setw(12) << _rho * (2.0/3.0) * kenergy_temp + (_ptail*_npart + 48.0*virial/3.0)/_volume << endl;
  }
    // TOTAL SPIN ////////////////////////////////////////////////////////////////
  if (_sim_type > 1){
    for(int i = 0; i<_npart; i++){
  ...
  // MAGNETIZATION /////////////////////////////////////////////////////////////
  if (_measure_magnet){
    ofstream coutm("../OUTPUT/EQUILIBRATION/magnetization.dat",ios::app);
    coutm << setw(12) << step
          << setw(12) << spin_sum / double(_npart) << endl;
  }
    // SPECIFIC HEAT /////////////////////////////////////////////////////////////
  if(_measure_cv and _measure_tenergy){
    double tenergy_mean = _accu_tenergy / double(step);
    ofstream coutcv("../OUTPUT/EQUILIBRATION/specific_heat.dat",ios::app);
    coutcv << setw(12) << step
          << setw(12) << (pow(tenergy_temp * _npart ,2) - pow(tenergy_mean * _npart, 2)) * pow(_beta,2) / _npart  << endl; 
  }  
  // SUSCEPTIBILITY //////////////////////////////////////////////////////////// 
  if(_measure_chi){
    ofstream coutchi("../OUTPUT/EQUILIBRATION/susceptibility.dat",ios::app);
    coutchi << setw(12) << step
          << setw(12) << _beta * pow(spin_sum ,2) / _npart << endl;
  } 

  // ACCEPTANCE ////////////////////////////////////////////////////////////////
  double fraction;
  ofstream coutf;
  coutf.open("../OUTPUT/EQUILIBRATION/acceptance.dat",ios::app);
  if(_nattempts > 0) fraction = double(_naccepted)/double(_nattempts);
  else fraction = 0.0; 
  coutf << setw(12) << step << setw(12) << fraction << endl;
  coutf.close();
...
// measure properties and save for each block in /OUTPUT/{prop}.dat
// use this function to measure properties during the simulation with one T
void System :: averages(int blk){
  ...
  _average     = _block_av / double(_nsteps);
	...
	if (_measure_cv){
    // block average of E*E - (block average of E)^2
  ...
  if (_measure_pofv){
    ...
    for (int b = 0; b < _n_bins_v; ++b) {
        average  = _average(_index_pofv + b);
        sum_average = _global_av(_index_pofv + b);
        sum_ave2 = _global_av2(_index_pofv + b);
        coutf << setw(12) << blk
              << setw(12) << b * _bin_size_v  // Centro del bin
              << setw(12) << average
              << setw(12) << sum_average / double(blk)
              << setw(12) << this->error(sum_average, sum_ave2, blk) << endl;
    }
  ...
  if (_measure_chi) {
    ...
    double average  = _average(_index_chi);
    double sum_average = _global_av(_index_chi);
    double sum_ave2    = _global_av2(_index_chi);
    coutf << setw(12) << blk
             << setw(12) << average
             << setw(12) << sum_average/double(blk)
             << setw(12) << this->error(sum_average, sum_ave2, blk) << endl;
  ...
  if (_measure_magnet) {
    ...
    double average  = _average(_index_magnet);
    double sum_average = _global_av(_index_magnet);
    double sum_ave2    = _global_av2(_index_magnet);
    coutf << setw(12) << blk
             << setw(12) << average
             << setw(12) << sum_average/double(blk)
             << setw(12) << this->error(sum_average, sum_ave2, blk) << endl;
  ...
  if (_measure_cv) {
    ...
    double average  = _average(_index_cv);
    double sum_average = _global_av(_index_cv);
    double sum_ave2    = _global_av2(_index_cv);
    coutf << setw(12) << blk
...
// finalize the simulation for each T
// and print the last averages with errors  
void System :: averages_final(int blk, double T){
  ...
  if(_nblocks  == blk){
    // print in output file the last averages
    // POTENTIAL ENERGY //////////////////////////////////////////////////////////
    if (_measure_penergy){
      coutf.open("../OUTPUT/potential_energy.dat",ios::app);
      average  = _average(_index_penergy);
    ...
    coutf << setw(12) << T << setw(12) << fraction << endl;
    coutf.close();
  }
...
double System :: error(double acc, double acc2, int blk){
  if(blk <= 1) return 0.0;
  else return sqrt( fabs(acc2/double(blk) - pow( acc/double(blk) ,2) )/double(blk) );  // non dovrebbe essere tutto diviso per (blk-1) al posto di blk
}
...
void System :: set_temperature(double T){
  ...
  ofstream coutf;
  coutf.open("../OUTPUT/output.dat",ios::app);
  coutf << "New simulation with temperature T = " << _temp << endl;
  _nattempts = 0;
  _naccepted = 0;
  return;
}
