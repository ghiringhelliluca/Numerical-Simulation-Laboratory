...
// system è un field, ovvero un vettore di particelle in una scatola con pbc

private:
  const int _ndim = 3;  // Dimensionality of the system
  bool _restart;        // Flag indicating if the simulation is restarted
  int _sim_type;        // Type of simulation (e.g., Lennard-Jones, Ising), 0 dinamica molecolare, 1 lennard jones con MC, 1 spin con MC
  ...
  int _eqsteps;         // Number of equilibration step for T = 2
  ...
  bool _measure_penergy, _measure_kenergy, _measure_tenergy;// Flags for measuring different energies, 0 msuro, 1 la misuro e così per tutte
  ...
  bool _measure_pofv;                                       // Flag for measuring the velocity modulus distribution, probabilità che devo inserire
  ...
  double _accu_tenergy; // Accumulator for total energy to compute specific heat
  // accumulatore di blocco per le osservabili
...
public: 
  ...
  int get_eqsteps();          // Get the number of eq step for T = 2
  void set_temperature(double T); // Set the temperature
  void initialize();          // Initialize system properties
  void initialize_equilibration(); // Initialize system for equilibration
  void initialize_properties();// Initialize properties for measurement
  void initialize_properties_equilibration(); // Initialize properties for equilibration measurement
  void initialize_properties_output(); // Initialize properties for final output for each T
  void finalize(double T);            // Finalize system and clean up
  void finalize_equilibration(); // Finalize equilibration
  ...
  void measure_equilibration(int step); // Measure properties during equilibration
  void averages(int blk);     // Compute averages of properties
  void averages_final(int blk, double T); // Output final averages to file
