...
//    for(unsigned int i=0; i<_ncity; i++){
//       if(i == _ncity - 1){
//          double theta1 = _individual(i)(1);
//          double theta2 = _individual(0)(1);    
//          double dtheta = fmod(theta2 - theta1 + 2*M_PI, 2*M_PI); // dtheta in [0, 2π), fmod(number, divisor) = number - divisor * floor(number/divisor), floor = round down
//          if (dtheta > M_PI) dtheta = 2*M_PI - dtheta;            // shortest arc
//          loss += _r * dtheta; 
//       } else{
//          double theta1 = _individual(i)(1);
//          double theta2 = _individual(i+1)(1);    
//          double dtheta = fmod(theta2 - theta1 + 2*M_PI, 2*M_PI); // dtheta in [0, 2π)
//          if (dtheta > M_PI) dtheta = 2*M_PI - dtheta;            // shortest arc
//          loss += _r * dtheta;  
//       }
...
double Individual::loss_circle(){
   ...
   for(unsigned int i=0; i<_ncity; i++){
      if(i == _ncity - 1){
         double theta1 = _individual(i)(1);
         double theta2 = _individual(0)(1);
         double dx = _r * cos(theta2) - _r * cos(theta1);
         double dy = _r * sin(theta2) - _r * sin(theta1);
