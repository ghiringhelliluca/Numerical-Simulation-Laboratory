...
void Population::initialize() {
  ...
  while ( !input.eof() ){
    ...
    } else if (property == "PROB"){
      input >> _prob_crossover >> _prob_mutation_shift >> _prob_mutation_inversion >> _prob_mutation_swap_blocks >> _prob_permutation;
      coutf << "PROB_CROSSOVER= " << _prob_crossover << endl;
      coutf << "PROB_MUTATION_SHIFT= " << _prob_mutation_shift << endl;
      coutf << "PROB_MUTATION_INVERSION= " << _prob_mutation_inversion << endl;
      coutf << "PROB_MUTATION_SWAP_BLOCKS= " << _prob_mutation_swap_blocks << endl;
      coutf << "PROB_PERMUTATION= " << _prob_permutation << endl;
...
void Population::evolve() {
  ...
  for (int i = 0; i < _M; i += 2) {   // _M / 2
    ...
    // --- CROSSOVER ---
    if (_rnd.Rannyu() < _prob_crossover) { // > 50% crossover
    ...
    // --- MUTATION ---
    if (_rnd.Rannyu() < _prob_mutation_swap_blocks){
      child1.mutation_swap_blocks(_rnd);
    }
    
    if (_rnd.Rannyu() < _prob_mutation_shift){
       child1.mutation_shift(_rnd);
    }
    if (_rnd.Rannyu() < _prob_mutation_inversion) {
      child1.mutation_inversion(_rnd);
    }
    if (_rnd.Rannyu() < _prob_permutation) {
