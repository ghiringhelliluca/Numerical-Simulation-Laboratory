...
void System :: Verlet(){
  ...
  for(int i=0; i<_npart; i++){ //Force acting on particle i, force = acc because m=1
...
void System :: initialize_velocities(){
  ...
  if(_restart){ // if restart simulation from a previous run, read coord from conf-1.xyz
    ...
    // remuve center of mass motion to delate casual drift
    // in this way I decrease the temperature
    ...
    // to have the desired temperature 
    // removing drift, the quadratic sum of the components of the velocities
    // decreases, this is proportional to K, so K decreases and also T
    // average v^2 = 3T
    for ( int i=0; i<_npart; i++){
  ...
  if (input.is_open()){
    while ( !input.eof() ){
      ...
      if( property == "POTENTIAL_ENERGY" ){
        ...
        index_property++;
        double rc3 = pow(_r_cut,3);
        double rc9 = pow(rc3,3);
        double sr3 = 1 / rc3;
        double sr9 = 1 / rc9;
        _vtail = (8 * M_PI * _rho / 3) * (1/3 * sr9 - sr3);
        ...
        double sr3 = 1 / rc3;
        double sr9 = 1 / rc9;
        _ptail = (16 * M_PI * _rho*_rho / 3) * (2/3 * sr9 - sr3);
      } else if( property == "GOFR" ){
        ofstream coutgr("../OUTPUT/gofr.dat"); //final value of gofr
        coutgr << "# DISTANCE:     AVE_GOFR:        ERROR:" << endl;
        coutgr.close();
        ofstream coutgr2("../OUTPUT/gofr2.dat");
        coutgr2 << "# BLOCK:    DISTANCE:     GOFR_ISTANT:      CUM_AVE_GOFR:        ERROR:" << endl; // cumulative value of gofr
        coutgr2.close();
        input>>_n_bins;
        _nprop+=_n_bins;
        ...
        input >> _n_bins_v;
        _nprop += _n_bins_v;
        _bin_size_v = 4.0* sqrt(_temp)/(double)_n_bins_v; // use sqrt(T) to have a better distribution
        _measure_pofv = true;
...
void System :: measure(){ // Measure properties
  ...
  int gofr_temp[_n_bins];
  for (int i=0; i<_n_bins; i++) gofr_temp[i] = 0;
    ...
    for (int i=0; i<_npart-1; i++){
      for (int j=i+1; j<_npart; j++){
        ...
        if (dr < _halfside.min()) {
          ...
          gofr_temp[bin] += 2.0; // count two times (i,j) e (j,i)
        }
  ...
  if (_measure_gofr){
    ...
    for (int b = 0; b < _n_bins; ++b) {
      _measurement(_index_gofr + b) = 0.0;
    }
    // accumulate bins count
    for (int i = 0; i < _n_bins; ++i) {
      double r_lower = i * _bin_size;
      double r_upper = (i+1) * _bin_size;
      // double r_middle = (r_lower + r_upper) / 2.0;
      double shell_volume = (4.0/3.0) * M_PI * (pow(r_upper,3) - pow(r_lower,3));
      double normalization = _rho * _npart * shell_volume;
      _measurement(_index_gofr + i) = double(gofr_temp[i]) / (normalization ); // devo ancora normalizzare per il numero di misure nel blocco, lo faccio in average
    }
  }
  ...
  if (_measure_pressure) { 
    ...
    if (_sim_type == 1) _measurement[_index_pressure] = _rho * _temp + (_ptail*_npart + 48.0*virial/3.0)/_volume;}  // MAGNETIZATION /////////////////////////////////////////////////////////////
// TO BE FIXED IN EXERCISE 6
  // SPECIFIC HEAT /////////////////////////////////////////////////////////////
// TO BE FIXED IN EXERCISE 6
  // SUSCEPTIBILITY ////////////////////////////////////////////////////////////
...
void System :: averages(int blk){
  ...
  if (_measure_gofr) {
    if(blk == _nblocks){
      ofstream coutgr("../OUTPUT/gofr.dat", ios::app);
      for (int i = 0; i < _n_bins; ++i) {
        sum_average = _global_av(_index_gofr + i)  ;
        sum_ave2 = _global_av2(_index_gofr + i);
        coutgr << setw(12) << i * _bin_size  //  bin center
               << setw(12) << sum_average / double(blk)
               << setw(12) << this->error(sum_average, sum_ave2, blk) << endl;
      }
      coutgr << endl; // linea vuota tra blocchi
      coutgr.close();
    }
    coutf.open("../OUTPUT/gofr2.dat", ios::app);
    for (int i = 0; i < _n_bins; ++i) {
      average  = _average(_index_gofr + i) ;
      sum_average = _global_av(_index_gofr + i) ;
      sum_ave2 = _global_av2(_index_gofr + i) ;
      coutf << setw(12) << blk
            << setw(12) << i * _bin_size  //  bin center
            << setw(12) << average
            << setw(12) << sum_average/ double(blk)
            << setw(12) << this->error(sum_average, sum_ave2, blk) << endl;
    }
  ...
  if (_measure_pofv){
    ...
    for (int b = 0; b < _n_bins_v; ++b) {
        ...
        coutf << setw(12) << blk
              << setw(12) << b * _bin_size_v  //  bin center
              << setw(12) << average
              << setw(12) << sum_average / double(blk)
              << setw(12) << this->error(sum_average, sum_ave2, blk) << endl;
    }
